...
await this.sendMessage(m.chat, { text: `🚩 ${usuario} *Ha cambiado la imágen del grupo*`, mentions: [m.sender] }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100}) 
...
await this.sendMessage(m.chat, { text: `🚩 *Nuevo admin*\n\nNombre: @${m.messageStubParameters[0].split`@`[0]}\nLe otorgó admin: @${m.sender.split`@`[0]}`], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
} else if (m.messageStubType == 30) {
await this.sendMessage(m.chat, { text: `🚩 *Un admin menos*\n\nNombre: @${m.messageStubParameters[0].split`@`[0]}\nLe quitó admin: @${m.sender.split`@`[0]}`], mentions: (await conn.groupMetadata(m.chat)).participants.map(v => v.id) }, { quoted: fkontak, ephemeralExpiration: 24*60*100, disappearingMessagesInChat: 24*60*100})
